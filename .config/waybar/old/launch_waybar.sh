...
sed -i "/hwmon-path/c\"hwmon-path\": \"$HWMON_PATH\"," $SDIR/config1 && sed -i "/hwmon-path/c\"hwmon-path\": \"$HWMON_PATH\"," $SDIR/config1

waybar -c "$SDIR"/config1 -s "$SDIR"/style1.css -b bar-0 &
